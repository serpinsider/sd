...
export default function AirbnbQuoteForm() {
  ...
  const handleSubmit = async () => {
    ...
    const confirmNum = 'PC-AIRBNB-' + Math.random().toString(36).substring(2, 8).toUpperCase();
    const addons = [];
    ...
    try {
      const response = await fetch('https://formspree.io/f/mrbjzvde', {
        ...
        body: JSON.stringify({
          ...
          _subject: `San Diego Maids - Airbnb Quote from ${formData.firstName} ${formData.lastName} - #${confirmNum}`,

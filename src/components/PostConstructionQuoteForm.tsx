...
export default function PostConstructionQuoteForm() {
  ...
  const handleSubmit = async () => {
    ...
    const confirmNum = 'PC-POST-' + Math.random().toString(36).substring(2, 8).toUpperCase();
    ...
    try {
      const response = await fetch('https://formspree.io/f/mrbjzvde', {
        ...
        body: JSON.stringify({
          ...
          _subject: `San Diego Maids - Post-Construction Quote from ${formData.firstName} ${formData.lastName} - #${confirmNum}`,
    ...
    <div className="w-full max-w-full sm:container mx-auto px-4 py-20">
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
        <div className="lg:col-span-2 backdrop-blur-md p-8 rounded-xl shadow-xl border border-white/10" style={{ background: getFormBg() }}>
          <h1 className="text-3xl md:text-4xl font-serif font-bold text-white mb-2">Post-Construction Cleaning Quote</h1>
          ...
          <div className="space-y-6">
            ...
            <div><label className="block text-sm font-semibold mb-2 text-white">Additional Details (Optional)</label><textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} rows={3} placeholder="Type of construction, timeline, special requirements..." className="w-full p-3 border border-white/20 rounded-lg text-white placeholder-white/50 focus:border-[#dfbd69]" style={{ background: getFormBg() }} /></div>
            <p className="text-xs text-white/60 text-center mb-4 mt-8">By submitting this form, you agree to receive communications from San Diego Maids regarding your quote request.</p>
          ...
          <div className="space-y-8">
            <div>
              ...
              <div className="space-y-4">
                ...
                <div><span className="text-base font-semibold text-white">Property:</span><p className="text-base text-white/80">{formData.bedrooms || '?'} bed / {formData.bathrooms || '?'} bath</p></div>
              </div>
            </div>
            <div className="pt-6 border-t border-white/10">
              ...
              <div className="mt-6 space-y-4">
                <h4 className="text-base font-semibold text-white">Post-Construction FAQ</h4>
                <div className="space-y-4">
                  <div className="space-y-1"><p className="text-sm font-medium text-white/90">What's included?</p><p className="text-xs text-white/70 leading-relaxed">Dust removal, debris cleanup, window cleaning, fixture polishing, floor cleaning.</p></div>
                  <div className="space-y-1"><p className="text-sm font-medium text-white/90">When should I book?</p><p className="text-xs text-white/70 leading-relaxed">After construction is complete but before moving in or final inspection.</p></div>
                  <div className="space-y-1"><p className="text-sm font-medium text-white/90">How long does it take?</p><p className="text-xs text-white/70 leading-relaxed">Depends on size and condition. Usually 4-8 hours for standard homes.</p></div>
                </div>
              </div>
            </div>
          </div>

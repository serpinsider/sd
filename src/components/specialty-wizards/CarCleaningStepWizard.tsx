...
interface CarCleaningStepWizardProps {
  onFormExpand?: (expanded: boolean, immediate?: boolean) => void;
}

export default function CarCleaningStepWizard({ onFormExpand }: CarCleaningStepWizardProps) {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    vehicleType: '', vehicleMakeModel: '', serviceType: '', notes: '',
  });
  ...
  const handleCloseSuccess = () => { setCurrentStep(1); setFormData({ phone: '', firstName: '', lastName: '', email: '', zipCode: '', vehicleType: '', vehicleMakeModel: '', serviceType: '', notes: '' }); if (onFormExpand) onFormExpand(false); };
    ...
    try {
      ...
      const response = await fetch('https://formspree.io/f/xvzwolek', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          ...
          'Vehicle Make/Model': formData.vehicleMakeModel || 'Not provided',
          'Service Type': formData.serviceType || 'Not specified',
          'Notes': formData.notes || 'No additional notes', 
          'Confirmation Number': confirmationNumber,
          _subject: `San Diego Maids - Car Detailing Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 1:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h3 className="text-sm sm:text-base text-white/70 mb-2">Get a quote for car detailing</h3></div>
      ...
      case 2:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h3 className="text-lg sm:text-xl font-semibold text-white mb-2">Vehicle & Contact Details</h3></div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold mb-2 text-white">Vehicle Type</label>
                <div className="relative">
                  <select value={formData.vehicleType} onChange={(e) => updateFormData('vehicleType', e.target.value)} className="w-full p-3 pr-10 border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm">
                    <option value="">Select...</option><option value="Sedan">Sedan</option><option value="Coupe">Coupe</option><option value="SUV">SUV</option><option value="Truck">Truck</option><option value="Van/Minivan">Van/Minivan</option><option value="Luxury/Exotic">Luxury/Exotic</option><option value="Other">Other</option>
                  </select>
              ...
              <div><label className="block text-sm font-semibold mb-2 text-white">Make/Model</label><input type="text" value={formData.vehicleMakeModel} onChange={(e) => updateFormData('vehicleMakeModel', e.target.value)} placeholder="e.g. Toyota Camry 2023" className="w-full p-3 border border-white/20 rounded-lg bg-white/10 text-white placeholder-white/50 focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm" /></div>
            </div>
            <div>
              <label className="block text-sm font-semibold mb-2 text-white">Service Needed</label>
              <div className="relative">
                <select value={formData.serviceType} onChange={(e) => updateFormData('serviceType', e.target.value)} className="w-full p-3 pr-10 border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm">
                  <option value="">Select...</option><option value="Interior Only">Interior Only</option><option value="Exterior Only">Exterior Only</option><option value="Full Detail">Full Detail (Interior + Exterior)</option><option value="Express Wash">Express Wash</option><option value="Not Sure">Not Sure</option>
                </select>
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-white/70"><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></div>
              </div>
            </div>
            <div><label className="block text-sm font-semibold mb-2 text-white">Additional Details (Optional)</label><textarea value={formData.notes} onChange={(e) => updateFormData('notes', e.target.value)} rows={3} placeholder="Specific stains, pet hair, any concerns..." className="w-full p-3 border border-white/20 rounded-lg bg-white/10 text-white placeholder-white/50 focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm" /></div>
          </div>
        );
      case 3:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h3 className="text-lg sm:text-xl font-semibold text-white mb-2">Review Your Request</h3><p className="text-sm text-white/70">We'll contact you to discuss your car detailing needs</p></div>
            <div className="space-y-4 bg-white/10 backdrop-blur-sm rounded-lg p-6 border border-white/20">
              ...
              {formData.vehicleType && <div><span className="text-sm font-semibold text-white">Vehicle:</span><p className="text-sm text-white/80">{formData.vehicleType}{formData.vehicleMakeModel ? ` - ${formData.vehicleMakeModel}` : ''}</p></div>}
              {formData.serviceType && <div><span className="text-sm font-semibold text-white">Service:</span><p className="text-sm text-white/80">{formData.serviceType}</p></div>}
  ...
  if (currentStep === 4) return <SuccessMessage type="quote" confirmationNumber={formData.confirmationNumber} inline={true} onClose={handleCloseSuccess} serviceLabel="car detailing" />;

  return (
    <div className="w-full">
      <div className="text-center mb-6">
        <h2 className="text-xl sm:text-2xl font-serif font-semibold text-white mb-3 drop-shadow-lg">Car Detailing Quote</h2>

...
interface CarpetStepWizardProps {
  onFormExpand?: (expanded: boolean, immediate?: boolean) => void;
}

export default function CarpetStepWizard({ onFormExpand }: CarpetStepWizardProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    rooms: '',
    hasStairs: false, stairsCount: '',
    hasHallways: false, hallwaysCount: '',
    hasAreaRugs: false, areaRugsCount: '',
    hasPetStains: false,
    notes: '',
    confirmationNumber: '',
  });
  ...
  const handleCloseSuccess = () => { setCurrentStep(1); setFormData({ phone: '', firstName: '', lastName: '', email: '', zipCode: '', rooms: '', hasStairs: false, stairsCount: '', hasHallways: false, hallwaysCount: '', hasAreaRugs: false, areaRugsCount: '', hasPetStains: false, notes: '', confirmationNumber: '' }); if (onFormExpand) onFormExpand(false); };
  ...
  const handleSubmit = async (retryCount = 0) => {
    ...
    try {
      const confirmationNumber = 'PC-CARPET-' + Math.random().toString(36).substring(2, 8).toUpperCase();
      ...
      const response = await fetch('https://formspree.io/f/mrbjzvde', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          _subject: `San Diego Maids - Carpet Cleaning Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
    ...
    } catch (error) { const message = error instanceof Error ? error.message : 'Unknown error'; if (retryCount < 2 && (error instanceof TypeError || message.includes('fetch'))) { setTimeout(() => handleSubmit(retryCount + 1), 1000); return; } if (message.includes('fetch') || error instanceof TypeError) { setSubmitError('Network error. Please check your connection and try again.'); } else if (message.includes('400')) { setSubmitError('Please check your information and try again.'); } else { setSubmitError('Something went wrong. Please try again or contact us directly.'); } } finally { setIsSubmitting(false); }
  };
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 1:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h3 className="text-sm sm:text-base text-white/70 mb-2">Get a quote for carpet cleaning</h3></div>
      ...
      case 2:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h3 className="text-lg sm:text-xl font-semibold text-white mb-2">Contact & Carpet Details</h3></div>
            ...
            <div>
              ...
              <div className="relative">
                <select value={formData.rooms} onChange={(e) => updateFormData('rooms', e.target.value)} className="w-full p-3 pr-10 border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm">
                  <option value="">Select...</option><option value="1">1 Room</option><option value="2">2 Rooms</option><option value="3">3 Rooms</option><option value="4">4 Rooms</option><option value="5">5 Rooms</option><option value="6+">6+ Rooms</option><option value="Whole House">Whole House</option><option value="Not Sure">Not Sure</option>
                </select>
            ...
            <div className="space-y-3 pt-4 border-t border-white/20">
              <p className="text-sm font-semibold text-white mb-2">Additional Areas</p>
              <label className="flex items-center cursor-pointer"><input type="checkbox" checked={formData.hasStairs} onChange={(e) => updateFormData('hasStairs', e.target.checked)} className="w-4 h-4 rounded border-white/20 text-[#dfbd69] focus:ring-[#dfbd69] bg-white/10" /><span className="ml-3 text-sm text-white">Carpeted stairs</span></label>
              {formData.hasStairs && <div className="ml-7"><select value={formData.stairsCount} onChange={(e) => updateFormData('stairsCount', e.target.value)} className="w-32 p-2 text-sm border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] backdrop-blur-sm"><option value="">Select...</option><option value="1">1 Staircase</option><option value="2">2 Staircases</option><option value="3+">3+ Staircases</option></select></div>}
              <label className="flex items-center cursor-pointer"><input type="checkbox" checked={formData.hasHallways} onChange={(e) => updateFormData('hasHallways', e.target.checked)} className="w-4 h-4 rounded border-white/20 text-[#dfbd69] focus:ring-[#dfbd69] bg-white/10" /><span className="ml-3 text-sm text-white">Carpeted hallways</span></label>
              {formData.hasHallways && <div className="ml-7"><select value={formData.hallwaysCount} onChange={(e) => updateFormData('hallwaysCount', e.target.value)} className="w-32 p-2 text-sm border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] backdrop-blur-sm"><option value="">Select...</option><option value="1">1 Hallway</option><option value="2">2 Hallways</option><option value="3+">3+ Hallways</option></select></div>}
              <label className="flex items-center cursor-pointer"><input type="checkbox" checked={formData.hasAreaRugs} onChange={(e) => updateFormData('hasAreaRugs', e.target.checked)} className="w-4 h-4 rounded border-white/20 text-[#dfbd69] focus:ring-[#dfbd69] bg-white/10" /><span className="ml-3 text-sm text-white">Area rugs</span></label>
              {formData.hasAreaRugs && <div className="ml-7"><select value={formData.areaRugsCount} onChange={(e) => updateFormData('areaRugsCount', e.target.value)} className="w-32 p-2 text-sm border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] backdrop-blur-sm"><option value="">Select...</option><option value="1">1 Rug</option><option value="2">2 Rugs</option><option value="3">3 Rugs</option><option value="4+">4+ Rugs</option></select></div>}
              <label className="flex items-center cursor-pointer"><input type="checkbox" checked={formData.hasPetStains} onChange={(e) => updateFormData('hasPetStains', e.target.checked)} className="w-4 h-4 rounded border-white/20 text-[#dfbd69] focus:ring-[#dfbd69] bg-white/10" /><span className="ml-3 text-sm text-white">Pet stains or odors</span></label>
            </div>
            <div><label className="block text-sm font-semibold mb-2 text-white">Additional Details (Optional)</label><textarea value={formData.notes} onChange={(e) => updateFormData('notes', e.target.value)} rows={3} placeholder="Specific stains, concerns, or questions..." className="w-full p-3 border border-white/20 rounded-lg bg-white/10 text-white placeholder-white/50 focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm" /></div>
          </div>
        );
      case 3: {
        const extras = getExtras();
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h3 className="text-lg sm:text-xl font-semibold text-white mb-2">Review Your Request</h3><p className="text-sm text-white/70">We'll contact you to discuss your carpet cleaning needs</p></div>
            <div className="space-y-4 bg-white/10 backdrop-blur-sm rounded-lg p-6 border border-white/20">
              ...
              {formData.rooms && <div><span className="text-sm font-semibold text-white">Rooms:</span><p className="text-sm text-white/80">{formData.rooms}</p></div>}
              {extras.length > 0 && <div><span className="text-sm font-semibold text-white">Extras:</span><p className="text-sm text-white/80">{extras.join(', ')}</p></div>}
  ...
  if (currentStep === 4) return <SuccessMessage type="quote" confirmationNumber={formData.confirmationNumber} inline={true} onClose={handleCloseSuccess} serviceLabel="carpet cleaning" />;

  return (
    <div className="w-full">
      <div className="text-center mb-6">
        <h2 className="text-xl sm:text-2xl font-serif font-semibold text-white mb-3 drop-shadow-lg">Carpet Cleaning Quote</h2>

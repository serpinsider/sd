...
export default function CarpetStepWizard({ onFormExpand }: CarpetStepWizardProps) {
  ...
  const handleCloseSuccess = () => { setCurrentStep(1); setFormData({ phone: '', firstName: '', lastName: '', email: '', roomsWithCarpet: '1', notes: '' }); if (onFormExpand) onFormExpand(false); };
    ...
    try {
      ...
      const response = await fetch('https://formspree.io/f/mrbjzvde', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          ...
          roomsWithCarpet: formData.roomsWithCarpet, 
      ...
      case 2:
        return (
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-semibold mb-2 text-white">Rooms with Carpet</label>
              <div className="relative">
                <select value={formData.roomsWithCarpet} onChange={(e) => updateFormData('roomsWithCarpet', e.target.value)} className="w-full p-3 pr-10 border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm">
                  <option value="1">1 Room</option><option value="2">2 Rooms</option><option value="3">3 Rooms</option><option value="4">4 Rooms</option><option value="5+">5+ Rooms</option><option value="Whole House">Whole House</option>
                </select>
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-4 bg-white/10 backdrop-blur-sm rounded-lg p-6 border border-white/20">
              ...
              <div><span className="text-sm font-semibold text-white">Rooms with Carpet:</span><p className="text-sm text-white/80">{formData.roomsWithCarpet}</p></div>
            ...
            <p className="text-[10px] sm:text-xs text-white/60 text-center leading-relaxed">By submitting, you agree to receive communications from San Diego Maids regarding your quote request.</p>
          </div>
        );
      default: return null;
    }
  ...
  return (
    <div className="w-full">
      <div className="text-center mb-6"><h2 className="text-xl sm:text-2xl font-serif font-bold text-white mb-4 drop-shadow-lg">Carpet Cleaning Quote</h2></div>

...
export default function CommercialStepWizard({ onFormExpand }: CommercialStepWizardProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    businessName: '', businessType: '', squareFootage: '', frequency: '',
  ...
  const handleCloseSuccess = () => { setCurrentStep(1); setFormData({ phone: '', firstName: '', lastName: '', email: '', zipCode: '', businessName: '', businessType: '', squareFootage: '', frequency: '', notes: '', confirmationNumber: '' }); if (onFormExpand) onFormExpand(false); };
    ...
    try {
      ...
      const response = await fetch('https://formspree.io/f/xvzwolek', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          _subject: `San Diego Maids - Commercial Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 2:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h3 className="text-lg sm:text-xl font-semibold text-white mb-2">Contact & Business Details</h3></div>
            ...
            <div><label className="block text-sm font-semibold mb-2 text-white">Business Name</label><input type="text" value={formData.businessName} onChange={(e) => updateFormData('businessName', e.target.value)} placeholder="Your Business Name" className="w-full p-3 border border-white/20 rounded-lg bg-white/10 text-white placeholder-white/50 focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm" /></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold mb-2 text-white">Business Type</label>
                <div className="relative">
                  <select value={formData.businessType} onChange={(e) => updateFormData('businessType', e.target.value)} className="w-full p-3 pr-10 border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm">
                    <option value="">Select...</option><option value="Office">Office</option><option value="Retail">Retail Store</option><option value="Medical">Medical/Dental</option><option value="Restaurant">Restaurant</option><option value="Warehouse">Warehouse</option><option value="Gym/Fitness">Gym/Fitness Center</option><option value="Other">Other</option><option value="Not Sure">Not Sure</option>
                  </select>
              ...
              <div>
                ...
                <div className="relative">
                  <select value={formData.squareFootage} onChange={(e) => updateFormData('squareFootage', e.target.value)} className="w-full p-3 pr-10 border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm">
                    <option value="">Select...</option><option value="Under 1,000">Under 1,000 sqft</option><option value="1,000-2,500">1,000-2,500 sqft</option><option value="2,500-5,000">2,500-5,000 sqft</option><option value="5,000-10,000">5,000-10,000 sqft</option><option value="10,000+">10,000+ sqft</option><option value="Not Sure">Not Sure</option>
                  </select>
                  <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-white/70"><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></div>
                </div>
              </div>
            ...
            <div>
              <label className="block text-sm font-semibold mb-2 text-white">Cleaning Frequency</label>
              <div className="relative">
                <select value={formData.frequency} onChange={(e) => updateFormData('frequency', e.target.value)} className="w-full p-3 pr-10 border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm">
                  <option value="">Select...</option><option value="Daily">Daily</option><option value="3x/week">3 times per week</option><option value="2x/week">2 times per week</option><option value="Weekly">Weekly</option><option value="Bi-weekly">Bi-weekly</option><option value="Monthly">Monthly</option><option value="One-time">One-time</option><option value="Not Sure">Not Sure</option>
                </select>
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-white/70"><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></div>
              </div>
            </div>
            <div><label className="block text-sm font-semibold mb-2 text-white">Additional Details (Optional)</label><textarea value={formData.notes} onChange={(e) => updateFormData('notes', e.target.value)} rows={3} placeholder="Special requirements, access details, or questions..." className="w-full p-3 border border-white/20 rounded-lg bg-white/10 text-white placeholder-white/50 focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm" /></div>
          </div>
        );
      case 3:
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-4 bg-white/10 backdrop-blur-sm rounded-lg p-6 border border-white/20">
              ...
              {formData.businessType && <div><span className="text-sm font-semibold text-white">Type:</span><p className="text-sm text-white/80">{formData.businessType}</p></div>}
              ...
              {formData.frequency && <div><span className="text-sm font-semibold text-white">Frequency:</span><p className="text-sm text-white/80">{formData.frequency}</p></div>}
  ...
  if (currentStep === 4) return <SuccessMessage type="quote" confirmationNumber={formData.confirmationNumber} inline={true} onClose={handleCloseSuccess} serviceLabel="commercial cleaning" />;

  return (
    <div className="w-full">
      <div className="text-center mb-6">
        <h2 className="text-xl sm:text-2xl font-serif font-semibold text-white mb-3 drop-shadow-lg">Commercial Cleaning Quote</h2>

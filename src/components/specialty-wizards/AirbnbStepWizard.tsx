...
export default function AirbnbStepWizard({ onFormExpand }: AirbnbStepWizardProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    propertyType: '', bedrooms: '', bathrooms: '', turnoverFrequency: '',
    laundryLinens: false, restocking: false, applianceCleaning: false,
  ...
  const handleCloseSuccess = () => { setCurrentStep(1); setFormData({ phone: '', firstName: '', lastName: '', email: '', zipCode: '', propertyType: '', bedrooms: '', bathrooms: '', turnoverFrequency: '', laundryLinens: false, restocking: false, applianceCleaning: false, notes: '', confirmationNumber: '' }); if (onFormExpand) onFormExpand(false); };
  ...
  const handleSubmit = async (retryCount = 0) => {
    ...
    try {
      const confirmationNumber = 'PC-AIRBNB-' + Math.random().toString(36).substring(2, 8).toUpperCase();
      ...
      const response = await fetch('https://formspree.io/f/xvzwolek', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          _subject: `San Diego Maids - Airbnb Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
      ...
      case 1:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h3 className="text-sm sm:text-base text-white/70 mb-2">Get a quote for Airbnb turnover cleaning</h3></div>
      ...
      case 2:
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold mb-2 text-white">Property Type</label>
                <div className="relative">
                  <select value={formData.propertyType} onChange={(e) => updateFormData('propertyType', e.target.value)} className="w-full p-3 pr-10 border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm">
                    <option value="">Select...</option><option value="Studio">Studio</option><option value="Apartment">Apartment</option><option value="Condo">Condo</option><option value="House">House</option><option value="Townhouse">Townhouse</option><option value="Not Sure">Not Sure</option>
                  </select>
              ...
              <div>
                ...
                <div className="relative">
                  <select value={formData.turnoverFrequency} onChange={(e) => updateFormData('turnoverFrequency', e.target.value)} className="w-full p-3 pr-10 border border-white/20 rounded-lg bg-white/10 text-white appearance-none focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm">
                    <option value="">Select...</option><option value="Weekly">Weekly</option><option value="2-3 times/week">2-3 times/week</option><option value="Daily">Daily</option><option value="As needed">As needed</option><option value="Not Sure">Not Sure</option>
                  </select>
                  <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-white/70"><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></div>
                </div>
              </div>
            ...
            <div className="space-y-3 pt-4 border-t border-white/20">
              <p className="text-sm font-semibold text-white mb-2">Add-on Services</p>
              <label className="flex items-center cursor-pointer"><input type="checkbox" checked={formData.laundryLinens} onChange={(e) => updateFormData('laundryLinens', e.target.checked)} className="w-4 h-4 rounded border-white/20 text-[#dfbd69] focus:ring-[#dfbd69] bg-white/10" /><span className="ml-3 text-sm text-white">Laundry/Linens</span></label>
              <label className="flex items-center cursor-pointer"><input type="checkbox" checked={formData.restocking} onChange={(e) => updateFormData('restocking', e.target.checked)} className="w-4 h-4 rounded border-white/20 text-[#dfbd69] focus:ring-[#dfbd69] bg-white/10" /><span className="ml-3 text-sm text-white">Restock Supplies</span></label>
              <label className="flex items-center cursor-pointer"><input type="checkbox" checked={formData.applianceCleaning} onChange={(e) => updateFormData('applianceCleaning', e.target.checked)} className="w-4 h-4 rounded border-white/20 text-[#dfbd69] focus:ring-[#dfbd69] bg-white/10" /><span className="ml-3 text-sm text-white">Appliance Cleaning</span></label>
            </div>
            <div><label className="block text-sm font-semibold mb-2 text-white">Additional Details (Optional)</label><textarea value={formData.notes} onChange={(e) => updateFormData('notes', e.target.value)} rows={3} placeholder="Access codes, special instructions, or questions..." className="w-full p-3 border border-white/20 rounded-lg bg-white/10 text-white placeholder-white/50 focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm" /></div>
          </div>
        );
      case 3: {
        const addons = getAddons();
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h3 className="text-lg sm:text-xl font-semibold text-white mb-2">Review Your Request</h3><p className="text-sm text-white/70">We'll contact you to discuss your turnover cleaning needs</p></div>
            <div className="space-y-4 bg-white/10 backdrop-blur-sm rounded-lg p-6 border border-white/20">
              ...
              {formData.propertyType && <div><span className="text-sm font-semibold text-white">Property:</span><p className="text-sm text-white/80">{formData.propertyType} - {formData.bedrooms || '?'} bed / {formData.bathrooms || '?'} bath</p></div>}
              {formData.turnoverFrequency && <div><span className="text-sm font-semibold text-white">Frequency:</span><p className="text-sm text-white/80">{formData.turnoverFrequency}</p></div>}
              {addons.length > 0 && <div><span className="text-sm font-semibold text-white">Add-ons:</span><p className="text-sm text-white/80">{addons.join(', ')}</p></div>}
  ...
  if (currentStep === 4) return <SuccessMessage type="quote" confirmationNumber={formData.confirmationNumber} inline={true} onClose={handleCloseSuccess} serviceLabel="Airbnb cleaning" />;

  return (
    <div className="w-full">
      <div className="text-center mb-6">
        <h2 className="text-xl sm:text-2xl font-serif font-semibold text-white mb-3 drop-shadow-lg">Airbnb Turnover Quote</h2>

...
export default function HandymanStepWizard({ onFormExpand }: HandymanStepWizardProps) {
  ...
  const handleCloseSuccess = () => { setCurrentStep(1); setFormData({ phone: '', firstName: '', lastName: '', email: '', jobDescription: '' }); if (onFormExpand) onFormExpand(false); };
    ...
    try {
      ...
      const response = await fetch('https://formspree.io/f/mrbjzvde', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          ...
          confirmationNumber,
          _subject: `San Diego Maids - Handyman Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`, 
      ...
      case 2:
        return (
          <div className="space-y-6">
            ...
            <div><label className="block text-sm font-semibold mb-2 text-white">What do you need help with?</label><textarea value={formData.jobDescription} onChange={(e) => updateFormData('jobDescription', e.target.value)} rows={4} placeholder="TV mounting, furniture assembly, minor repairs, etc..." className="w-full p-3 border border-white/20 rounded-lg bg-white/10 text-white placeholder-white/50 focus:border-[#dfbd69] focus:ring-1 focus:ring-[#dfbd69] backdrop-blur-sm" /></div>
          </div>
        );
      case 3:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h3 className="text-lg sm:text-xl font-semibold text-white mb-2">Review Your Request</h3><p className="text-sm text-white/70">We'll contact you to discuss your project</p></div>
  ...
  return (
    <div className="w-full">
      <div className="text-center mb-6"><h2 className="text-xl sm:text-2xl font-serif font-bold text-white mb-4 drop-shadow-lg">Handyman Services Quote</h2></div>
